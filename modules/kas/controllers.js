...
const KasControllerList = async (req, res) => {
  try {
    ...
    // example:
    const results = Kas.find(KasFilter(req)).sort([["created", -1]]);
...
const KasControllerCreate = async (req, res) => {
  try {
    // Your code here
    const result = await Kas.create({...req.cleanedData, email: req.user.email,});
...
const KasControllerUpdate = async (req, res) => {
  try {
    ...
    const kas = await Kas.findOne({_id: req.params.id});
    if (!kas) throw {status: 404, message: "Not found"};
    const result = await Kas.findOneAndUpdate({_id: req.params.id}, req.cleanedData, {new: true});
